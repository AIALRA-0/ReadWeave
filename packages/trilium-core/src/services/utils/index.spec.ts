...
describe("#sanitizeSvg", () => {
    ...
    it("should remove malformed script closing tags", () => {
        ...
        expect(utils.sanitizeSvg(maliciousSvg)).toBe('<svg><rect width="100"/></svg>');
    });
    ...
    it("should remove foreign object content", () => {
        ...
        expect(utils.sanitizeSvg(maliciousSvg)).toBe('<svg><rect/></svg>');
    });

    it("should replace vbscript URLs with #", () => {
        expect(utils.sanitizeSvg('<svg><a href="vbscript:attack()">link</a></svg>'))
            .toBe('<svg><a href="#">link</a></svg>');
    });

    it("should replace non-image data URLs with #", () => {
        expect(utils.sanitizeSvg('<svg><a href="data:text/html;base64,PHNjcmlwdD4=">link</a></svg>'))
            .toBe('<svg><a href="#">link</a></svg>');
    });
    ...
    it("should preserve nested content in non-void SVG elements", () => {
        ...
        expect(utils.sanitizeSvg(validSvg)).toBe(validSvg);
    });
});
