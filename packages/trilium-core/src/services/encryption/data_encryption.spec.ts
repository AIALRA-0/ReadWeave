...
import { decodeBase64, decodeUtf8, encodeBase64, encodeUtf8 } from "../utils/binary.js";
...
const KEY = encodeUtf8("test-data-encryption-key");
...
describe("data_encryption", () => {
    describe("encrypt + decrypt round-trip", () => {
        ...
        it("produces a different ciphertext each time (random salt and IV) but both decrypt equally", () => {
    ...
    describe("decrypt edge cases", () => {
        ...
        it("rejects modified authenticated ciphertext", () => {
            ...
            const payload = decodeBase64(encrypted.slice("v2:".length));
            payload[payload.length - 1] ^= 1;

            expect(dataEncryption.decrypt(KEY, `v2:${encodeBase64(payload)}`)).toBe(false);
        });
