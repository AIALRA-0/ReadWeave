...
describe("SqlService (real DB)", () => {
    ...
    describe("insert / replace / upsert", () => {
        ...
        it("upsert inserts then updates on conflict and coerces booleans to 0/1", () => {
            ...
            const original = { id: 5, name: "orig", flag: true };

            getSql().upsert(table, "id", original);
            expect(getSql().getValue<number>(`SELECT flag FROM "${table}" WHERE id = 5`)).toBe(1);
            expect(original.flag).toBe(true);
        ...
        it("rejects dynamic SQL identifiers outside the allowlist grammar", () => {
            expect(() => getSql().insert("notes; DROP TABLE notes", { name: "x" })).toThrow(/Invalid SQL identifier/);
            expect(() => getSql().insert(createTempTable(), { "name) VALUES ('x'); --": "x" })).toThrow(/Invalid SQL identifier/);
            expect(() => getSql().upsert(createTempTable(), "id) DO UPDATE", { id: 1 })).toThrow(/Invalid SQL identifier/);
        });
    });
