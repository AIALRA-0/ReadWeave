...
If you have the possibility of rebuilding the source code (e.g. if a package maintainer), copy `apps/client/.env.example` to `apps/client/.env` and set `VITE_CKEDITOR_KEY=GPL` in the local `.env` file. The local file is ignored by Git.
