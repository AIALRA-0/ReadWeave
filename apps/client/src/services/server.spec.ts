...
describe("ajax error handling", () => {
    ...
    it("does not start authentication recovery before the initial password is set", async () => {
        (window as unknown as { glob: Record<string, unknown> }).glob = { dbInitialized: true, passwordSet: false };
        (window as unknown as { $: { ajax: (opts: AjaxOptions) => void } }).$.ajax = (opts: AjaxOptions) => {
            void opts.error({ status: 401, responseText: "Logged in session not found" });
        };

        await expect(server.get("options")).rejects.toBe("Logged in session not found");
        expect(authenticationRecoveryMock.recoverExpiredAuthentication).not.toHaveBeenCalled();
    });
