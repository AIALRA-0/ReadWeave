...
describe("activateParentNotePath (via deleteNotes navigation)", () => {
    it("moves every open tab away from a note before deleting it", async () => {
        const note = buildNote({ title: "Shared open note" });
        makeBranch("sharedBranch", note.noteId, "root");
        const firstSetNote = vi.fn(async () => {});
        const secondSetNote = vi.fn(async () => {});
        appContext.tabManager = {
            getActiveContext: () => ({ notePathArray: [ "root", note.noteId ], setNote: firstSetNote }),
            getNoteContexts: () => [
                { notePathArray: [ "root", note.noteId ], setNote: firstSetNote },
                { notePathArray: [ "root", note.noteId ], setNote: secondSetNote }
            ]
        } as any;
        appContext.triggerCommand = vi.fn((_name: any, data: any) => {
            data.callback({ proceed: true, deleteAllClones: false, eraseNotes: false });
        ...
        await branches.deleteNotes([ "sharedBranch" ]);
        expect(firstSetNote).toHaveBeenCalledWith("root");
        expect(secondSetNote).toHaveBeenCalledWith("root");
    });
