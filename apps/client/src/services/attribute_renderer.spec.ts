...
describe("formatValue (via label rendering)", () => {
    ...
    it("formats values using the canonical double-quoted form", async () => {
        // plain (matches the allowed-character regex) -> unquoted
        expect(await rendered("simple-value_1,2.3")).toBe("simple-value_1,2.3");
        // complex values are always wrapped in double quotes
        expect(await rendered("a b")).toBe(`"a b"`);
        // double quotes are escaped while apostrophes and backticks stay literal
        ...
        // backslashes are escaped before quotes so the rendered value round-trips
        expect(await rendered('path\\with"quote')).toBe(String.raw`"path\\with\"quote"`);
    });
});
