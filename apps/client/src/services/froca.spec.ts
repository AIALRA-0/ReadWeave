...
describe("loadInitialTree", () => {
    ...
    it("returns early while the initial password is not set", async () => {
        (globalThis as unknown as { glob: Record<string, unknown> }).glob = { dbInitialized: true, passwordSet: false };
        server.get = vi.fn(async () => {
            throw new Error("should not be called");
        }) as typeof server.get;
    ...
    it("clears and re-populates the cache from the tree endpoint when initialized", async () => {
        (globalThis as unknown as { glob: Record<string, unknown> }).glob = { dbInitialized: true, passwordSet: true, loggedIn: true };
