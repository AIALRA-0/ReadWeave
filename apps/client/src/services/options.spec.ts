...
describe("Options service", () => {
    ...
    it("keeps an empty local record when server options are intentionally not loaded", async () => {
        ...
        await fresh.initializedPromise;
        ...
        expect(fresh.is(k("anything"))).toBe(false);
        expect(() => fresh.set(k("anything"), "v")).not.toThrow();
        expect(fresh.get(k("anything"))).toBe("v");
