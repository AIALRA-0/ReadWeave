...
import type { EntityChange, EntityType } from "../server_types.js";
...
let ws: WebSocket;
let reconnectEnabled = isAuthenticatedAppReady();
...
const FRONTEND_ENTITY_TYPES = new Set<EntityType>([
    "notes",
    "branches",
    "attributes",
    "note_reordering",
...
function normalizeEntityChanges(entityChanges: Extract<WebSocketMessage, { type: "frontend-update" }>["data"]["entityChanges"]): EntityChange[] {
    return entityChanges.map((entityChange) => {
        if (!FRONTEND_ENTITY_TYPES.has(entityChange.entityName as EntityType)) {
            throw new Error(`Unsupported frontend entity type '${entityChange.entityName}'.`);
        }

        return {
            ...entityChange,
            entityName: entityChange.entityName as EntityType
        };
    });
}
...
export async function dispatchMessage(message: WebSocketMessage) {
    ...
    if (message.type === "ping") {
        ...
        if (message.protectedSessionAvailable === false && glob.isProtectedSessionAvailable) {
        ...
        await executeFrontendUpdate(normalizeEntityChanges(message.data.entityChanges));
        ...
        toastService.showMessage(message.message, message.timeout);
    } else if (message.type === "execute-script") {
        const originEntity = message.originEntityId ? await froca.getNote(message.originEntityId) : null;

        void bundleService.getAndExecuteBundle(message.currentNoteId, originEntity, message.script, message.params);
    }
}
...
async function consumeFrontendUpdateData() {
    if (frontendUpdateDataQueue.length > 0) {
        ...
        try {
            ...
            logError(`Encountered error ${detail.message}: ${detail.stack}, reloading frontend.`);
            ...
            if (!glob.isDev && !options.is("debugModeEnabled")) {
                ...
                toastService.showError(t("ws.encountered-error", { message: detail.message }));
            }
        }
