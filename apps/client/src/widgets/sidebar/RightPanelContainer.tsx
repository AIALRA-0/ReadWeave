...
import ReadWeavePanel from "./ReadWeavePanel";
...
function useItems(rightPaneVisible: boolean, widgetsByParent: WidgetsByParent) {
    ...
    const definitions: RightPanelWidgetDefinition[] = [
        ...
        {
            el: <ReadWeavePanel />,
            // ReadWeave currently relies on the server-side repository and AI
            // routes. Mounting it in standalone mode creates persistent 404
            // error toasts which cover the tab bar and break unrelated input.
            enabled: noteType === "text" && !isStandalone,
        },
        {
...
function useSplit(mode: PaneMode) {
    ...
    useLayoutEffect(() => {
        ...
        const minSize = mode === "docked" ? [ 300, MIN_RIGHT_PANE_WIDTH ] : [ 0, MIN_RIGHT_PANE_WIDTH ];
