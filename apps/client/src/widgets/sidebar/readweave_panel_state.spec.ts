...
describe("ReadWeave panel state", () => {
    ...
    it("keeps completed and failed drafts retryable but blocks active jobs", () => {
        ...
        expect(isReadWeaveGenerationDisabled({ ...base, jobStatus: "ready-for-review" })).toBe(false);
    ...
    it("polls the full generation history only while a job is active", () => {
        ...
        expect(hasActiveReadWeaveGenerationJobs([
            generationJob({ status: "ready-for-review" }),
    ...
    it("keeps exact-range emphasis only while a status indicator is present", () => {
        ...
        expect(readWeaveGenerationVisualState({ status: "ready-for-review", unread: false, qualityState: "provisional" })).toBe("paused");
    });

    it("removes an anchor when its final saved link and completed job are deleted together", () => {
        expect(shouldRemoveReadWeaveAnchorAfterDelete({
            anchorId: "anchor-1",
            deletedLinkIds: [ "link-1" ],
            entryLinkIds: [ "link-1" ],
            generationJobs: [ generationJob({ savedLinkId: "link-1" }) ]
        })).toBe(true);
    });

    it("keeps an anchor used by an independent draft or another saved entry", () => {
        expect(shouldRemoveReadWeaveAnchorAfterDelete({
            anchorId: "anchor-1",
            deletedLinkIds: [ "link-1" ],
            entryLinkIds: [ "link-1" ],
            generationJobs: [ generationJob({ savedLinkId: undefined, status: "running" }) ]
        })).toBe(false);
        expect(shouldRemoveReadWeaveAnchorAfterDelete({
            anchorId: "anchor-1",
            deletedLinkIds: [ "link-1" ],
            entryLinkIds: [ "link-1", "link-2" ],
            generationJobs: []
        })).toBe(false);
    });
    ...
    it("applies persistent tombstones even when a delayed snapshot still contains the deleted job", () => {
        const deleted = generationJob({ jobId: "deleted-job", stateVersion: 4 });
        const retained = generationJob({ jobId: "retained-job", stateVersion: 2 });

        expect(mergeReadWeaveGenerationJobSnapshot(
            [ deleted, retained ],
            [ { ...deleted, stateVersion: 3 } ],
            [ deleted.jobId ]
        )).toEqual([ retained ]);
    });
    ...
    it("creates one foldable failed log row when an old failed job has no progress", () => {
        ...
        expect(readWeaveGenerationProgressForDisplay(generationJob({ status: "ready-for-review" }), "生成失败。")).toEqual([]);
    });
