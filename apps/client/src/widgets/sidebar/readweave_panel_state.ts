...
export function hasActiveReadWeaveGenerationJobs(
    ...
    return jobs.some(job => job.status === "queued" || job.status === "running" || job.status === "saving");
}

/**
 * Remove an exact-range anchor after its final saved entry is deleted unless
 * an independent draft or another saved entry still uses it. A completed job
 * bound to the deleted link disappears in the same server transaction.
 */
export function shouldRemoveReadWeaveAnchorAfterDelete(input: {
    anchorId: string;
    deletedLinkIds: Iterable<string>;
    entryLinkIds: Iterable<string>;
    generationJobs: Pick<ReadWeaveGenerationJob, "anchorId" | "savedLinkId">[];
}): boolean {
    const deletedLinkIds = new Set(input.deletedLinkIds);
    const hasRemainingEntry = Array.from(input.entryLinkIds).some(linkId => !deletedLinkIds.has(linkId));
    const hasRemainingJob = input.generationJobs.some(job => job.anchorId === input.anchorId
        && (!job.savedLinkId || !deletedLinkIds.has(job.savedLinkId)));
    return !hasRemainingEntry && !hasRemainingJob;
}

export function isReadWeaveGenerationReviewable(status: ReadWeaveGenerationJob["status"] | undefined): boolean {
    return status === "ready-for-review" || status === "saved";
}
...
export function mergeReadWeaveGenerationJobSnapshot(
    ...
    }).filter(job => !removed.has(job.jobId));
    const incomingIds = new Set(incoming.map(job => job.jobId));
    const locallyNewerOrCrossArticle = current.filter(job => !incomingIds.has(job.jobId) && !removed.has(job.jobId));
...
export function recoverReadWeaveGenerationFields(input: {
    ...
    const completedResult = isReadWeaveGenerationReviewable(input.job?.status) ? input.job?.result : undefined;
...
function compareReadWeaveJobVersions(left: ReadWeaveGenerationJob, right: ReadWeaveGenerationJob): number {
    if (left.stateVersion !== right.stateVersion) return left.stateVersion - right.stateVersion;
