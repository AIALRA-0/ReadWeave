...
const premiumPluginsMock = vi.hoisted(() => vi.fn(async () => [ class PremiumPluginStub {} ]));

vi.mock("@triliumnext/ckeditor5", async (importOriginal) => ({
    ...(await importOriginal<typeof import("@triliumnext/ckeditor5")>()),
    loadPremiumPlugins: premiumPluginsMock
}));
...
describe("CK config - licensing", () => {
    ...
    it("enables premium plugins when a license key is available", async () => {
        vi.stubEnv("VITE_CKEDITOR_KEY", "test-premium-key");
        const config = await buildConfig(baseOpts({ forceGplLicense: false }));

        expect(config.licenseKey).toBe("test-premium-key");
        expect(Array.isArray(config.extraPlugins)).toBe(true);
        expect((config.extraPlugins ?? []).length).toBeGreaterThan(0);
        expect(premiumPluginsMock).toHaveBeenCalledOnce();
    });
});
