...
const inheritedDataDir = process.env.READWEAVE_AUTH_E2E_DATA_DIR;
const dataDir = inheritedDataDir ?? join(repoRoot, ".cache/readweave-auth-e2e");
...
// Playwright evaluates the config again inside each worker. The inherited
// directory marks those worker evaluations so they do not delete the database
// while the shared web server has it open.
if (!inheritedDataDir) {
