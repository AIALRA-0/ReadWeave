...
async function reloadIntoAuthenticationRecovery(page: import("@playwright/test").Page) {
    try {
        await page.reload({ waitUntil: "domcontentloaded", timeout: 15_000 });
    } catch (error: unknown) {
        // A protected startup request can discover the expired session while
        // the original reload is still in flight. Recovery intentionally uses
        // location.replace(), which aborts that first navigation. The login
        // assertion after this helper still proves the replacement completed.
...
test("expired login state returns to login, stops startup transports, and recovers after sign-in", async ({ page, context, baseURL }) => {
    ...
    page.on("request", request => {
        ...
        if (request.method() === "GET" && protectedStartupPaths.has(path)) startupRequests.push(path);
    });
    page.on("websocket", () => {
        webSocketAttempts += 1;
    });
    ...
    webSocketAttempts = 0;
    startedAt = Date.now();
    await reloadIntoAuthenticationRecovery(page);
    await waitForLogin(page);
