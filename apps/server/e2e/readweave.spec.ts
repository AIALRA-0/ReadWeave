...
async function ensureGeneratedItemSaved(panel: Locator) {
    ...
    await expect.poll(async () => (await panel.locator(".readweave-status").allTextContents()).join(" | ")).toContain("Reviewed item saved");
...
test("ReadWeave completes range anchoring, reviewed Q&A, term definition, reuse, editing, hover and export", async ({ page, context }) => {
    ...
    const reviewAndSave = panel.getByRole("button", { name: "I reviewed it — save", exact: true });
    await expect(reviewAndSave).toBeVisible();
    ...
    await expect(failedMonitor).toContainText("任务数据异常，已停止执行");
    ...
    await expect(failedIssueGroup).toContainText("ReadWeave 测试故障：定点修复重试已耗尽；未保存任何内容");
    await expect(failedMonitor).toContainText("定点修复重试已耗尽；未保存任何内容");
...
test("ReadWeave confirms a pending selection from the right panel and enables generation", async ({ page, context }) => {
    ...
    await expect(restoredPanel.getByRole("button", { name: "I reviewed it — save", exact: true })).toBeVisible();
...
test("ReadWeave starts generation directly from a pending selection without an extra confirmation click", async ({ page, context }) => {
    ...
    await expect(panel.getByRole("button", { name: "I reviewed it — save", exact: true })).toBeVisible();
});
...
test("ReadWeave ignores a delayed generation response after switching to another anchor", async ({ page, context }) => {
    ...
    try {
        ...
        }, { timeout: 10_000 }).toBe("ready-for-review");
...
test("ReadWeave handles diverse source articles and keeps cross-article term references synchronized", async ({ page, context }) => {
    ...
    await expect(quicAnchor).toHaveClass(/readweave-anchor-locked/);
    await page.mouse.move(5, 5);
    await expect.poll(() => quicAnchor.evaluate(element => getComputedStyle(element).textDecorationLine)).toContain("underline");
    await quicAnchor.dispatchEvent("click");
    await expect(quicAnchor).not.toHaveClass(/readweave-anchor-locked/);
    await page.mouse.move(5, 5);
    await expect.poll(() => quicAnchor.evaluate(element => getComputedStyle(element).textDecorationLine)).toBe("none");
    ...
    await expect(tessAnchor).toHaveClass(/readweave-anchor-locked/);
    await page.mouse.move(5, 5);
    await expect.poll(() => tessAnchor.evaluate(element => getComputedStyle(element).textDecorationLine)).toContain("underline");
    await tessAnchor.dispatchEvent("click");
    await expect(tessAnchor).not.toHaveClass(/readweave-anchor-locked/);
    await page.mouse.move(5, 5);
    await expect.poll(() => tessAnchor.evaluate(element => getComputedStyle(element).textDecorationLine)).toBe("none");
