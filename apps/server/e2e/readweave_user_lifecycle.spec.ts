...
test("user note lifecycle does not leave idle ReadWeave polling behind", async ({ page, baseURL }) => {
    if (!baseURL) throw new Error("Playwright baseURL is required for the lifecycle test");
    const appUrl = baseURL.endsWith("/") ? baseURL : `${baseURL}/`;
    const apiUrl = (path: string) => new URL(path, appUrl).toString();
    ...
    const created = await page.request.post(apiUrl("api/notes/root/children?target=into&targetBranchId="), {
    ...
    try {
        ...
        await expect.poll(async () => {
            const response = await page.request.get(apiUrl(`api/notes/${noteId}/blob`));
        ...
        if (noteId && csrfToken) {
            await page.request.delete(apiUrl(`api/notes/${noteId}?taskId=cleanup-${Date.now()}&last=true`), {
                headers: { "x-csrf-token": csrfToken }
            });
        }
    }
