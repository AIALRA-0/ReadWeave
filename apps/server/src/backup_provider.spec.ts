...
describe("ServerBackupService", () => {
    ...
    it.each([ "", "../escape", "nested/name", "name with spaces", "a".repeat(65) ])("rejects an unsafe backup name: %j", async (name) => {
        await expect(service.backupNow(name)).rejects.toThrow("Invalid backup name");
    });
});
