...
interface HarnessTrialJobRow {
    trialJobId: string;
    versionId: string;
    revisionId: string;
    contentDigest: string;
    status: ReadWeaveHarnessTrialJob["status"];
...
const activeTrialJobs = new Set<string>();
...
function contentDigest(modules: ReadWeaveHarnessModules, cases: ReadWeaveHarnessCase[]): string {
    return createHash("sha256").update(JSON.stringify({ modules, cases })).digest("hex");
}
...
function insertRevision(
    ...
    const digest = contentDigest(modules, cases);
    const createdAt = new Date().toISOString();
    sql.execute(/* sql */`
        INSERT INTO readweave_harness_revisions (
            revisionId, versionId, parentRevisionId, contentDigest, modulesJson, casesJson, createdAt
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [ revisionId, versionId, parentRevisionId ?? null, digest, JSON.stringify(modules), JSON.stringify(cases), createdAt ]);
    return { revisionId, versionId, parentRevisionId, contentDigest: digest, modules, cases, createdAt };
}
...
function ensureStorage() {
    ...
    sql.executeScript(/* sql */`
        ...
        CREATE TABLE IF NOT EXISTS readweave_harness_revisions (
        ...
        CREATE INDEX IF NOT EXISTS readweave_harness_revisions_version
            ON readweave_harness_revisions(versionId, createdAt DESC);
        CREATE TABLE IF NOT EXISTS readweave_harness_trial_jobs (
            trialJobId TEXT PRIMARY KEY,
            versionId TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS readweave_harness_trial_jobs_status
            ON readweave_harness_trial_jobs(status, updatedAt);
        CREATE TABLE IF NOT EXISTS readweave_harness_publications (
    ...
    `);
    const profileColumns = sql.getColumn<string>("SELECT name FROM pragma_table_info('readweave_harness_profiles')");
    if (!profileColumns.includes("currentRevisionId")) sql.execute("ALTER TABLE readweave_harness_profiles ADD COLUMN currentRevisionId TEXT");
    if (!profileColumns.includes("contentDigest")) sql.execute("ALTER TABLE readweave_harness_profiles ADD COLUMN contentDigest TEXT");
    const count = sql.getValue<number>("SELECT COUNT(*) FROM readweave_harness_profiles") ?? 0;
    if (count === 0) {
        const timestamp = new Date().toISOString();
        const revision = insertRevision(DEFAULT_VERSION, DEFAULT_READWEAVE_HARNESS_MODULES, DEFAULT_CASES);
        sql.execute(/* sql */`
            INSERT INTO readweave_harness_profiles (
                versionId, name, status, parentVersionId, currentRevisionId, contentDigest, modulesJson, casesJson,
                lastTrialJson, createdAt, updatedAt, publishedAt
            ) VALUES (?, ?, 'legacy-published', NULL, ?, ?, ?, ?, NULL, ?, ?, ?)
        `, [
            DEFAULT_VERSION,
            "ReadWeave 质量闭环 v2",
            revision.revisionId,
        ...
        if (!defaultRow) {
            const published = sql.getRowOrNull<HarnessRow>(
                "SELECT * FROM readweave_harness_profiles WHERE status IN ('published', 'legacy-published') ORDER BY publishedAt DESC LIMIT 1"
            );
            const timestamp = new Date().toISOString();
            const revision = insertRevision(DEFAULT_VERSION, DEFAULT_READWEAVE_HARNESS_MODULES, DEFAULT_CASES);
            sql.execute(/* sql */`
    ...
    const legacyRows = sql.getRows<HarnessRow>("SELECT * FROM readweave_harness_profiles WHERE currentRevisionId IS NULL OR contentDigest IS NULL");
    for (const row of legacyRows) {
        const modules = parseJson<ReadWeaveHarnessModules>(row.modulesJson);
        const cases = parseJson<ReadWeaveHarnessCase[]>(row.casesJson);
        const revision = insertRevision(row.versionId, modules, cases);
        sql.execute("UPDATE readweave_harness_profiles SET currentRevisionId = ?, contentDigest = ?, status = CASE WHEN status = 'published' THEN 'legacy-published' ELSE status END WHERE versionId = ?", [
            revision.revisionId, revision.contentDigest, row.versionId
        ]);
    }
    sql.execute("UPDATE readweave_harness_trial_jobs SET status = 'queued', updatedAt = ? WHERE status = 'running'", [ new Date().toISOString() ]);
    storageReady = true;
    for (const row of sql.getRows<HarnessTrialJobRow>("SELECT * FROM readweave_harness_trial_jobs WHERE status = 'queued' ORDER BY createdAt")) {
        scheduleTrialJob(row.trialJobId);
    }
}
...
function profileFor(row: HarnessRow): ReadWeaveHarnessProfile {
    const revision = sql.getRowOrNull<HarnessRevisionRow>("SELECT * FROM readweave_harness_revisions WHERE revisionId = ?", [ row.currentRevisionId ]);
    if (!revision || revision.versionId !== row.versionId || revision.contentDigest !== row.contentDigest) {
        throw new ValidationError("ReadWeave Harness 当前修订损坏");
    }
    return {
        ...
        modules: parseJson<ReadWeaveHarnessModules>(revision.modulesJson),
        cases: parseJson<ReadWeaveHarnessCase[]>(revision.casesJson),
...
export function getPublishedReadWeaveHarnessProfile(): ReadWeaveHarnessProfile {
    ensureStorage();
    const row = sql.getRowOrNull<HarnessRow>("SELECT * FROM readweave_harness_profiles WHERE status IN ('published', 'legacy-published') ORDER BY publishedAt DESC LIMIT 1");
...
export function createReadWeaveHarnessDraft(input: { sourceVersionId?: unknown; name?: unknown }): ReadWeaveHarnessProfile {
    ...
    const revision = insertRevision(versionId, source.modules, source.cases);
    sql.execute(/* sql */`
        INSERT INTO readweave_harness_profiles (
            versionId, name, status, parentVersionId, currentRevisionId, contentDigest, modulesJson, casesJson,
            lastTrialJson, createdAt, updatedAt, publishedAt
        ) VALUES (?, ?, 'draft', ?, ?, ?, ?, ?, NULL, ?, ?, NULL)
    `, [ versionId, name, source.versionId, revision.revisionId, revision.contentDigest, JSON.stringify(source.modules), JSON.stringify(source.cases), timestamp, timestamp ]);
...
export function updateReadWeaveHarnessDraft(versionId: string, input: { name?: unknown; modules?: unknown; cases?: unknown }): ReadWeaveHarnessProfile {
    ...
    const revision = insertRevision(versionId, modules, cases, existing.currentRevisionId);
    sql.execute(/* sql */`
        UPDATE readweave_harness_profiles
        SET name = ?, status = 'draft', currentRevisionId = ?, contentDigest = ?, modulesJson = ?, casesJson = ?, lastTrialJson = NULL, updatedAt = ?
        WHERE versionId = ?
    `, [ name, revision.revisionId, revision.contentDigest, JSON.stringify(modules), JSON.stringify(cases), new Date().toISOString(), versionId ]);
    return getReadWeaveHarnessProfile(versionId);
}
...
function trialJobFor(row: HarnessTrialJobRow): ReadWeaveHarnessTrialJob {
    return {
        ...
        result: row.resultJson ? parseTrial(row.resultJson) : undefined,
...
function profileAtRevision(versionId: string, revisionId: string, digest: string): ReadWeaveHarnessProfile {
    const row = rowFor(versionId);
    const revision = sql.getRowOrNull<HarnessRevisionRow>("SELECT * FROM readweave_harness_revisions WHERE revisionId = ?", [ revisionId ]);
    if (!revision || revision.versionId !== versionId || revision.contentDigest !== digest) {
        throw new ValidationError("Harness 试跑修订不存在或摘要不匹配");
    }
    return {
        ...profileFor(row),
        currentRevisionId: revision.revisionId,
...
export function trialReadWeaveHarnessProfile(versionId: string, input: unknown): ReadWeaveHarnessTrialJob {
    if (input && typeof input === "object" && !Array.isArray(input) && Object.keys(input).length > 0) {
        throw new ValidationError("Harness 试跑不接受调用方提供答案或其他覆盖内容");
    }
    const profile = getReadWeaveHarnessProfile(versionId);
    if (profile.status !== "draft" && profile.status !== "trial") throw new ValidationError("只有可编辑 Harness 版本可以试跑");
    const trialJobId = `trial-${randomUUID()}`;
    const timestamp = new Date().toISOString();
    const totalCases = profile.cases.length + READWEAVE_HOLDOUT_QUALITY_CASES.length;
    sql.execute(/* sql */`
    ...
    `, [ trialJobId, versionId, profile.currentRevisionId, profile.contentDigest, totalCases, timestamp, timestamp ]);
    scheduleTrialJob(trialJobId);
    return getReadWeaveHarnessTrialJob(trialJobId);
}

export function getReadWeaveHarnessTrialJob(trialJobId: string): ReadWeaveHarnessTrialJob {
    ensureStorage();
    const row = sql.getRowOrNull<HarnessTrialJobRow>("SELECT * FROM readweave_harness_trial_jobs WHERE trialJobId = ?", [ trialJobId ]);
    if (!row) throw new ValidationError("Harness 试跑任务不存在");
    return trialJobFor(row);
}

export function getLatestReadWeaveHarnessTrialJob(versionId: string): ReadWeaveHarnessTrialJob | undefined {
    rowFor(versionId);
    const row = sql.getRowOrNull<HarnessTrialJobRow>(/* sql */`
        SELECT * FROM readweave_harness_trial_jobs
        WHERE versionId = ? ORDER BY createdAt DESC LIMIT 1
    `, [ versionId ]);
    return row ? trialJobFor(row) : undefined;
}

export function initializeReadWeaveHarnessTrials() {
    ensureStorage();
    sql.execute("UPDATE readweave_harness_trial_jobs SET status = 'queued', updatedAt = ? WHERE status = 'running'", [ new Date().toISOString() ]);
    for (const row of sql.getRows<HarnessTrialJobRow>("SELECT * FROM readweave_harness_trial_jobs WHERE status = 'queued' ORDER BY createdAt")) {
        scheduleTrialJob(row.trialJobId);
...
function scheduleTrialJob(trialJobId: string) {
    if (activeTrialJobs.has(trialJobId)) return;
    activeTrialJobs.add(trialJobId);
    setImmediate(() => void runTrialJob(trialJobId).finally(() => activeTrialJobs.delete(trialJobId)));
}

async function runTrialJob(trialJobId: string): Promise<void> {
    const claimed = sql.execute(/* sql */`
        UPDATE readweave_harness_trial_jobs
        SET status = 'running', updatedAt = ?
        WHERE trialJobId = ? AND status = 'queued'
    `, [ new Date().toISOString(), trialJobId ]);
    if (claimed.changes !== 1) return;
    const trialRow = sql.getRow<HarnessTrialJobRow>("SELECT * FROM readweave_harness_trial_jobs WHERE trialJobId = ?", [ trialJobId ]);
    const profile = profileAtRevision(trialRow.versionId, trialRow.revisionId, trialRow.contentDigest);
    ...
    try {
        const outcomes = await mapWithConcurrency(cases, 2, async testCase => {
            ...
            completedCases += 1;
            sql.execute("UPDATE readweave_harness_trial_jobs SET completedCases = ?, updatedAt = ? WHERE trialJobId = ? AND status = 'running'", [
                completedCases, new Date().toISOString(), trialJobId
            ]);
        });
        ...
        const result: ReadWeaveHarnessTrialResult = {
            versionId: trialRow.versionId,
            revisionId: trialRow.revisionId,
            contentDigest: trialRow.contentDigest,
        ...
        const timestamp = new Date().toISOString();
        sql.transactional(() => {
            sql.execute("UPDATE readweave_harness_trial_jobs SET status = ?, completedCases = ?, resultJson = ?, error = NULL, updatedAt = ? WHERE trialJobId = ?", [
                result.passed ? "passed" : "failed", cases.length, JSON.stringify(result), timestamp, trialJobId
            ]);
            sql.execute(/* sql */`
                UPDATE readweave_harness_profiles
                SET status = ?, lastTrialJson = ?, updatedAt = ?
                WHERE versionId = ? AND currentRevisionId = ? AND contentDigest = ?
            `, [ result.passed ? "trial" : "draft", JSON.stringify(result), timestamp, trialRow.versionId, trialRow.revisionId, trialRow.contentDigest ]);
        });
    } catch (error) {
        sql.execute("UPDATE readweave_harness_trial_jobs SET status = 'failed', error = ?, updatedAt = ? WHERE trialJobId = ?", [
            error instanceof Error ? error.message : "Harness 试跑失败", new Date().toISOString(), trialJobId
        ]);
    }
}

async function mapWithConcurrency<T, R>(items: T[], concurrency: number, worker: (item: T) => Promise<R>, afterEach?: () => void): Promise<R[]> {
...
export function publishReadWeaveHarnessProfile(versionId: string, inputValue: unknown): ReadWeaveHarnessProfile {
    const row = rowFor(versionId);
    if (row.status !== "trial") throw new ValidationError("只有已经通过试跑的 Harness 版本可以发布");
    if (!inputValue || typeof inputValue !== "object" || Array.isArray(inputValue)) throw new ValidationError("Harness 发布参数无效");
    const input = inputValue as { trialJobId?: unknown; revisionId?: unknown; contentDigest?: unknown };
    const trialJobId = requireText(input.trialJobId, "trialJobId", 128);
    const revisionId = requireText(input.revisionId, "revisionId", 128);
    const digest = requireText(input.contentDigest, "contentDigest", 128);
    const trialRow = sql.getRowOrNull<HarnessTrialJobRow>("SELECT * FROM readweave_harness_trial_jobs WHERE trialJobId = ?", [ trialJobId ]);
    if (!trialRow || trialRow.status !== "passed" || trialRow.versionId !== versionId) {
        throw new ValidationError("Harness 必须先通过真实后台试跑");
    }
    if (trialRow.revisionId !== revisionId || trialRow.contentDigest !== digest
        || row.currentRevisionId !== revisionId || row.contentDigest !== digest) {
        throw new ValidationError("Harness 已在试跑后发生变化，必须重新试跑");
    }
    const timestamp = new Date().toISOString();
    sql.transactional(() => {
        sql.execute("UPDATE readweave_harness_profiles SET status = 'archived', updatedAt = ? WHERE status IN ('published', 'legacy-published')", [ timestamp ]);
        const updated = sql.execute(/* sql */`
            UPDATE readweave_harness_profiles
            SET status = 'published', publishedAt = ?, updatedAt = ?
            WHERE versionId = ? AND status = 'trial' AND currentRevisionId = ? AND contentDigest = ?
        `, [ timestamp, timestamp, versionId, revisionId, digest ]);
        if (updated.changes !== 1) throw new ValidationError("Harness 发布竞争失败，请重新载入后重试");
        sql.execute("INSERT INTO readweave_harness_publications (versionId, revisionId, contentDigest, publishedAt) VALUES (?, ?, ?, ?)", [
            versionId, revisionId, digest, timestamp
        ]);
    });
    return getReadWeaveHarnessProfile(versionId);
}
...
export function rollbackReadWeaveHarnessProfile(versionId: string): ReadWeaveHarnessProfile {
    ...
    const publication = sql.getRowOrNull<{ revisionId: string; contentDigest: string }>(/* sql */`
        SELECT revisionId, contentDigest FROM readweave_harness_publications
        WHERE versionId = ? ORDER BY sequence DESC LIMIT 1
    `, [ versionId ]);
    if (!publication || publication.revisionId !== source.currentRevisionId || publication.contentDigest !== source.contentDigest) {
        throw new ValidationError("只有真实发布历史中的未变更版本可以回滚");
    }
    const timestamp = new Date().toISOString();
    sql.transactional(() => {
        sql.execute("UPDATE readweave_harness_profiles SET status = 'archived', updatedAt = ? WHERE status IN ('published', 'legacy-published')", [ timestamp ]);
        ...
        sql.execute("INSERT INTO readweave_harness_publications (versionId, revisionId, contentDigest, publishedAt) VALUES (?, ?, ?, ?)", [
            versionId, publication.revisionId, publication.contentDigest, timestamp
        ]);
    });
    return getReadWeaveHarnessProfile(versionId);
}
...
export function archiveReadWeaveHarnessProfile(versionId: string): ReadWeaveHarnessProfile {
    const profile = getReadWeaveHarnessProfile(versionId);
    if (profile.status === "published" || profile.status === "legacy-published") throw new ValidationError("当前发布版本不能直接归档，请先发布其他版本");
