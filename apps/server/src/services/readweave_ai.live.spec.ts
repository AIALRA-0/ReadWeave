...
const PROFESSIONAL_LABELS = [ "定义与命名", "底层构造", "层次关系", "参数配置", "行为语义", "测试判据", "数字推导", "实现选择与证据闭环" ];

function expectNaturalDirectAnswer(body: string, professional = true): void {
    ...
    if (professional) {
        let previous = -1;
        for (const label of PROFESSIONAL_LABELS) {
            const index = body.indexOf(`${label}：`);
            expect(index).toBeGreaterThan(previous);
            expect(body.split(`${label}：`)).toHaveLength(2);
            previous = index;
        }
    }
}
...
describeLive("ReadWeave live AI quality", () => {
    it("defines an abbreviation with the canonical bilingual form", async () => {
        ...
        expectNaturalDirectAnswer(result.body, false);
    ...
    it("answers the DragonCat default-and-fallback question with a complete professional evidence loop", async () => {
        const result = await generateReadWeaveAnswer({
            articleId: "live-quality",
        ...
        expect(result.body).toMatch(/9\s*[-−]\s*6\s*=\s*3|3\s*秒/);
        ...
        expect(result.body).toMatch(/通过|失败|判定/);
    }, 180_000);
});
